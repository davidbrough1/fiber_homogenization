...
dataset3, stresses3 = make_elastic_stress_random(n_samples=n_samples,
                                                 ...
                                                 poissons_ratio=poissons_ratio,
                                                 macro_strain=macro_strain,
                                                 seed=0, power=power[2])
dataset4, stresses4 = make_elastic_stress_random(n_samples=n_samples,
                                                 size=size,
                                                 grain_size=grain_size,
                                                 elastic_modulus=elastic_modulus,
                                                 poissons_ratio=poissons_ratio,
                                                 macro_strain=macro_strain,
                                                 seed=0, power=power[3])
'''
# Long_fiber_x = make_microstructure(n_samples=sample_size, size=size,
...
#                                     n_phases=n_phases,
grain_size=grain_size4,
#                                     seed=3)
'''
dataset = np.concatenate((dataset1, dataset2, dataset3, dataset4), axis=0)
stresses = np.concatenate((stresses1, stresses2, stresses3, stresses4), axis=0)
